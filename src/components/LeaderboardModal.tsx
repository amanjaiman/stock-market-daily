            ...
            <form onSubmit={handleNameSubmit} className="space-y-4">
              ...
              <div className="flex gap-4">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 font-medium py-4 px-6 rounded-3xl floating-button bounce-click transition-all duration-200 bg-slate-600 hover:bg-slate-700 text-white"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 font-medium py-4 px-6 rounded-3xl floating-button bounce-click transition-all duration-200 bg-blue-500 hover:bg-blue-600 text-white"
                >
                  Continue
                </button>
              </div>
        ...
        {!isEnteringName && userName && (
          <div className="space-y-6">
            <div className="space-y-3">
              <h2 className="text-2xl font-bold text-slate-700 dark:text-slate-300 text-center">
                Leaderboard
              </h2>

              {/* Info Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-white dark:bg-slate-800 rounded-2xl px-4 py-3 text-center">
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-100 dark:bg-slate-950 border-b-2 border-gray-200 dark:border-slate-700">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider w-16">
                          Tries
                        </th>
                      </tr>
                    </thead>
                      ...
                      {leaderboardData.slice(0, 50).map((entry, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-6 py-2.5 text-sm font-medium text-slate-900 dark:text-slate-100">
                              {entry.name}
                              {isCurrentUser && (
                            ...
                            <td className="px-6 py-2.5 text-sm text-right whitespace-nowrap">
                              <div
                              ...
                              >
                                ${entry.final_value.toFixed(2)}
                              </div>
                              <div className="text-xs text-slate-500 dark:text-slate-400">
                                {entry.percentage_change_of_value >= 0
                                  ? "+"
                                  : ""}
                                {entry.percentage_change_of_value.toFixed(2)}%
                              </div>
                            </td>
                            <td className="px-6 py-2.5 text-sm text-right font-semibold text-slate-700 dark:text-slate-300 whitespace-nowrap">
                              ${entry.ppt.toFixed(2)}
                            </td>
                            <td className="px-6 py-2.5 text-sm text-right text-slate-600 dark:text-slate-400">
                              {entry.num_tries}
                            </td>
                          </tr>
                        );
            ...
            <div className="flex justify-center">
              <button
                onClick={onClose}
                className="font-medium py-4 px-8 rounded-3xl floating-button bounce-click transition-all duration-200 bg-blue-500 hover:bg-blue-600 text-white"
              >
                Close
              </button>
            </div>
